...
- The data is generated by a true underlying distribution, characterized by a CDF <img src="https://latex.codecogs.com/gif.latex?\inline&space;\dpi{300}&space;F(\mathbf{Y|X})" title="F(\mathbf{Y|X})" />, i.e. <img src="https://latex.codecogs.com/gif.latex?\inline&space;\dpi{300}&space;F(k|x)&space;=&space;p_{\mathbf{Y}|x}(\mathbf{Y}&space;\leq&space;k)" title="F(k|x) = p_{\mathbf{Y}|x}(\mathbf{Y} \leq k)" />
