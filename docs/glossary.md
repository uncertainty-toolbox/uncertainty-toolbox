...
- \(\mathbf{X, Y}\) are random variables and \(x, y\) are observed values of the random variables.
- Observations for \(\mathbf{Y}\) are always assumed to be conditioned on an observation of \(\mathbf{X}\).
- Dataset \(D = {(x_1, y_1), (x_2, y_2), ..., (x_n, y_n)}\),
\(x \in \mathbb{R}^{d}, y \in \mathbb{R}\)
- The data is generated by a true underlying distribution, characterized by a CDF \(F(\mathbf{Y|X})\), i.e. \(F(k|x) = p_{\mathbf{Y}|x}(\mathbf{Y} \leq k)\)
- The inverse function of this true CDF is the true quantile function, \(Q\), \(F(y|x) = p\Leftrightarrow Q(p|x) = y\)
- Uncertainty predictions are expressed as predictions of the quantile function \(\hat{Q}(p|x)\)
